...
const STORAGE_BASE = 'https://nefwqxatlmyiapkeadwo.supabase.co/storage/v1/object/public/brand-assets'
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6">
      <div className="text-center max-w-md">
        <h1 className="font-display text-3xl font-extrabold text-ink mb-3">{title}</h1>
...
export default async function ResultsPage({ params }: { params: { id: string } }) {
  const { data: session } = await supabase
    .from('brand_sessions')
    .select('brief, full_name, expires_at, signature_logo_path, monogram_logo_path')
    .eq('id', params.id)
    .single()

  if (!session) {
    return <Gate title="Link not found" message="This link isn't valid or the results have been removed." />
  }

  if (session.expires_at && new Date(session.expires_at) < new Date()) {
    return <Gate title="Link expired" message="This link has expired. Results links are active for 14 days after generation." />
  }

  const brief = session.brief as Brief
  const firstName = (session.full_name as string)?.split(' ')[0]

  const signatureUrl = session.signature_logo_path ? `${STORAGE_BASE}/${session.signature_logo_path}` : null
  const monogramUrl = session.monogram_logo_path ? `${STORAGE_BASE}/${session.monogram_logo_path}` : null
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-ink leading-tight">
            {firstName ? `${firstName}, this is the` : 'This is the'}{' '}
        ...
        {brief.brandInOneSentence && (
          <div className="bg-ink rounded-2xl p-8 md:p-10 mb-12 shadow-lg">
            ...
            <p className="font-display text-xl md:text-2xl text-bone leading-snug">{brief.brandInOneSentence}</p>
          </div>
        )}
        ...
        {brief.bio && <Section number="01" title="Your Bio">{paras(brief.bio)}</Section>}
        {brief.originStory && <Section number="02" title="Your Origin Story">{paras(brief.originStory)}</Section>}
        {brief.idealClient && <Section number="03" title="Your Ideal Client">{paras(brief.idealClient)}</Section>}
            ...
            {brief.brandVoice.descriptors && (
              <div className="flex flex-wrap gap-2 mb-5">
                {brief.brandVoice.descriptors.map((d, i) => (
                  <span key={i} className="font-body text-sm bg-emerald/15 text-emerald rounded-full px-3 py-1">{d}</span>
                ))}
              </div>
            )}
            ...
            {brief.brandVoice.examples && (
              <div className="mt-5 space-y-2">
                ...
                {brief.brandVoice.examples.map((ex, i) => (
                  <p key={i} className="font-display italic text-ink/80 border-l-2 border-coral pl-4">&ldquo;{ex}&rdquo;</p>
                ))}
              </div>
            )}
          </Section>
        ...
        {brief.pointOfView && <Section number="05" title="Your Point of View">{paras(brief.pointOfView)}</Section>}
        ...
        {brief.websiteImagePlan && <Section number="09" title="Your Website Image Plan">{paras(brief.websiteImagePlan)}</Section>}

        {brief.socialCaptions && (
          <Section number="10" title="Ready-to-Post Captions">
            <div className="space-y-4">
              {brief.socialCaptions.map((cap, i) => (
                <div key={i} className="bg-white/60 rounded-xl p-4 font-body text-ink/85 leading-relaxed whitespace-pre-wrap">{cap}</div>
              ))}
            </div>
          </Section>
        )}
        ...
        {(signatureUrl || monogramUrl) && (
          <Section number="12" title="Signature Logos">
            <p className="font-body text-ink/70 leading-relaxed mb-6">
              Your personal signature marks — ready to download as transparent PNGs.
            </p>
            <div className="space-y-8">
              {signatureUrl && (
                <div>
                  <h3 className="font-display text-lg font-bold text-ink mb-1">The Signature</h3>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={signatureUrl} alt="Signature logo" className="w-full max-w-md rounded-xl border border-black/5 bg-white mb-3" />
                  <a href={signatureUrl} download="signature-logo.png"
                    className="btn-emboss inline-block px-4 py-2 rounded-full bg-ink text-bone font-body text-sm font-bold">
                    Download PNG
                  </a>
                </div>
              )}
              {monogramUrl && (
                <div>
                  <h3 className="font-display text-lg font-bold text-ink mb-1">The Monogram</h3>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={monogramUrl} alt="Monogram logo" className="w-full max-w-md rounded-xl border border-black/5 bg-white mb-3" />
                  <a href={monogramUrl} download="monogram-logo.png"
                    className="btn-emboss inline-block px-4 py-2 rounded-full bg-ink text-bone font-body text-sm font-bold">
                    Download PNG
                  </a>
                </div>
